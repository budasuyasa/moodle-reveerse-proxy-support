...
RUN apt-get update && apt-get install -y \
  ...
  libonig-dev libcurl4-openssl-dev libxslt1-dev supervisor vim \
  && docker-php-ext-configure gd --with-freetype --with-jpeg \
  && docker-php-ext-install gd mysqli zip intl opcache soap xsl xmlrpc mbstring curl exif pdo pdo_mysql \
